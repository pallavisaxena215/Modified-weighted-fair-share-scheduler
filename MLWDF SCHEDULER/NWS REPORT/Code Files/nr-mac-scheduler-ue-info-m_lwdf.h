 ...
 */
class NrMacSchedulerUeInfoMLWDF : public NrMacSchedulerUeInfo
{
public:
  /**
   ...
   */
  NrMacSchedulerUeInfoMLWDF (float alpha, uint16_t rnti, BeamId beamId, const GetRbPerRbgFn &fn)
  ...
  void SetDropProb (double dropInput);
  ...
  // {
  //   auto luePtr = dynamic_cast<NrMacSchedulerUeInfoMLWDF*> (lue.first.get ());
  //   auto ruePtr = dynamic_cast<NrMacSchedulerUeInfoMLWDF*> (rue.first.get ());
  ...
  //   //NrEesmErrorModel::GetSpectralEfficiencyForCqi (luePtr->m_dlCqi.m_cqi);
  //   //std::cout<<p.m_rlcTransmissionQueueHolDelay<<std::endl;
  ...
  CompareUeWeightsDl (const NrMacSchedulerNs3::UePtrAndBufferReq &lue,
                      const NrMacSchedulerNs3::UePtrAndBufferReq &rue )
  {
    int m_prob = ns3::dropProbns3;
    
    //NrMacSchedSapProvider::SchedDlRlcBufferReqParameters p;
    //m_HolDelay = p.m_rlcTransmissionQueueHolDelay;
     
    auto luePtr = dynamic_cast<NrMacSchedulerUeInfoMLWDF *> (lue.first.get ());
    auto ruePtr = dynamic_cast<NrMacSchedulerUeInfoMLWDF *> (rue.first.get ());
    
    
    double lPfMetric = -log(m_prob)+(luePtr->m_tDelay) *(luePtr->m_HolDelay) *std::pow (luePtr->m_potentialTputDl, luePtr->m_alpha) / std::max (1E-9, luePtr->m_avgTputDl);
    double rPfMetric = -log(m_prob)+(ruePtr->m_tDelay )*(ruePtr->m_HolDelay)*std::pow (ruePtr->m_potentialTputDl, ruePtr->m_alpha) / std::max (1E-9, ruePtr->m_avgTputDl);
  ...
  /*
  void 
  ComputeSchedulingMetric(const NrMacSchedulerNs3::UePtrAndBufferReq &lue,
                          const NrMacSchedulerNs3::UePtrAndBufferReq &rue)
  {
    auto luePtr = dynamic_cast<NrMacSchedulerUeInfoMLWDF*> (lue.first.get ());
    auto ruePtr = dynamic_cast<NrMacSchedulerUeInfoMLWDF*> (rue.first.get ());
    ...
    NrMacSchedSapProvider::SchedDlRlcBufferReqParameters p;
    m_HolDeay = p.m_rlcTransmissionQueueHolDelay;
    std::cout<<m_HolDeay<<std::endl;

  //double a = (-log10 (qos->GetDropProbability())) / qos->GetMaxDelay ();
  //double HOL = bearer->GetHeadOfLinePacketDelay ();
	//metric = (a * HOL)*((spectralEfficiency * 180000.)/GetAverageTransmissionRate ());
  ...
  double metric {0.0};
  double m_HolDelay {99.0};
  double m_availableRate {0.0};
  double m_dProb {0.0};
  double m_tDelay {0.1};
  //PieQueueDisc pie;
};
