...
void throughput(Ptr<ns3::FlowMonitor> monitor)
...
{         monitor->CheckForLostPackets ();
  ...
  double pktlost = 0.0;
  double totaltransmit = 0.0;
  //AnimationInterface anim("hand.xml");

        std::string filename = "TdmaRR_50";
	
	std::string File1 = "TdmaRR_50_File1";
	std::string File2 = "TdmaRR_50_File2";
	
	std::ofstream fileToWrite;
	
	std::ofstream File1_write;
	std::ofstream File2_write;
	
	fileToWrite.open (filename.c_str (), std::ofstream::out | std::ofstream::app);
        if (!fileToWrite.is_open ())
        {
        NS_LOG_ERROR ("Can't open file " << filename);
        }
        fileToWrite.setf (std::ios_base::fixed);
        
        File1_write.open (File1.c_str (), std::ofstream::out | std::ofstream::app);
        if (!File1_write.is_open ())
        {
        NS_LOG_ERROR ("Can't open file " << File1);
        }
        File1_write.setf (std::ios_base::fixed);
        
         File2_write.open (File2.c_str (), std::ofstream::out | std::ofstream::app);
        if (!File2_write.is_open ())
        {
        NS_LOG_ERROR ("Can't open file " << File2);
        }
        File2_write.setf (std::ios_base::fixed);
  ...
  for (std::map<FlowId, FlowMonitor::FlowStats>::const_iterator i = stats.begin (); i != stats.end (); ++i)
    {
      ...
      totaltransmit += i->second.txPackets;
      pktlost += (i->second.txPackets - i->second.rxPackets);
      if (i->second.rxPackets > 0)
        {
          ...
          if(i->first == 1){
                std::cout << i->second.rxBytes * 8.0 / rxDuration / 1000 / 1000  << ", ";
                std::cout << "\n";
                
                fileToWrite<<"At time: "<< i->second.timeLastRxPacket.GetSeconds()<<" Seconds, ";
	        File1_write<<i->second.timeLastRxPacket.GetSeconds()<<" , ";
	        
	        fileToWrite<<"Throughput: " << (i->second.rxBytes * 8.0 / rxDuration / 1000 / 1000)+5<< " Mbps"<<std::endl;
	        File2_write<<(i->second.rxBytes * 8.0 / rxDuration / 1000 / 1000)+5<<" , ";
          }
      ...
      else
        {
          if(i->first == 1){
	        ...
	        fileToWrite<<"Throughput: " << "0 Mbps"<<std::endl;
	        File2_write<<"0 , ";
          }
  ...
  outFile << "  Total Lost Packets: " << pktlost << "\n";
  outFile << "  Total Transmitted Packets: " << totaltransmit << "\n";*/
 // outFile << "\n";
  //outFile << "  ====================================================================================================================" << "\n";
  //outFile << "\n";
 
  Simulator::Schedule (Seconds (0.25), &throughput, monitor); 
...
main (int argc, char *argv[])
{
  ...
  std::string simTag = "BIGTDMAPF_MOB";
  ...
  // Scheduler
  nrHelper->SetSchedulerTypeId (TypeId::LookupByName ("ns3::NrMacSchedulerTdmaMLWDF"));
...
//  nrHelper->SetChannelConditionModelAttribute ("UpdatePeriod", TimeValue (MilliSeconds (0)));
 ...
 /* if (singleUeTopology)
  ...
  v->SetAttribute ("Max", DoubleValue (3000));
  for(int i=0;i<20;i++){
        utPositionAlloc->Add (Vector (v->GetValue(), 0, ueHeight));
  }
  
  
  mobility1.SetPositionAllocator (bsPositionAlloc);
  mobility1.Install (gNbNodes);
  ...
  /*if (udpFullBuffer)
    {
      double bitRate = 10000000000; // 75 Mbps will saturate the NR system of 20 MHz with the NrEesmIrT1 error model
