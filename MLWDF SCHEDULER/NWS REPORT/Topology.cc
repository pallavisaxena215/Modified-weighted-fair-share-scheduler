...
main (int argc, char *argv[])
{
 ...
 double centralFrequency = 6e9;//for 0,1,2
  // double centralFrequency = 28e9;//for 0,1,2
  ...
  std::string simTag = "default1_speed10";
  std::string outputDir = "./";
   std::string SchedulerType ="ns3::NrMacSchedulerTdmaMLWDF";
...
//  nrHelper->SetChannelConditionModelAttribute ("UpdatePeriod", TimeValue (MilliSeconds (0)));
  ...
  utPositionAlloc->Add (Vector (1000, 1, ueHeight));
  utPositionAlloc->Add (Vector (3000, 1, ueHeight));
  utPositionAlloc->Add (Vector (-10, 1, ueHeight));
  utPositionAlloc->Add (Vector (-1000, -1, ueHeight));
  utPositionAlloc->Add (Vector (-3000,-1, ueHeight));
...
// Band40: CC0 - BWP0 & Band38: CC1 - BWP1
  ...
  p2ph.SetChannelAttribute ("Delay", TimeValue (MilliSeconds (5.000)));
  ...
  for (std::map<FlowId, FlowMonitor::FlowStats>::const_iterator i = stats.begin (); i != stats.end (); ++i)
    {
      ...
      out+=i->second.rxPackets;
    }

  outFile << "\n\n  Mean flow throughput: " << averageFlowThroughput / stats.size () << "\n";
  outFile << "  Mean flow delay: " << averageFlowDelay / stats.size () << "\n";
   outFile << "  Mean packet loss org: " << out - in << "\n";
  outFile << "  Total TxPackets: " << TotalTxPackets << "\n";
  outFile << "  Total RxPackets: " << TotalRxPackets << "\n";
